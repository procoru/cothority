...
func (bft *ProtocolBFTCoSi) handleCommitment(msgs []commitChan) error {
	...
	for _, msg := range msgs {
		...
		case RoundPrepare:
			...
			commitment = bft.prepare.Commit(bft.Random, bft.tempPrepareCommit)
			if bft.IsRoot() {
		...
		case RoundCommit:
			...
			commitment = bft.commit.Commit(bft.Random, bft.tempCommitCommit)
			if bft.IsRoot() {
				// do nothing:
...
func (bft *ProtocolBFTCoSi) startCommitment(t RoundType) error {
	cm := bft.getCosi(t).CreateCommitment(bft.Random)
