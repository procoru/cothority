...
// skipchain-service.
func (c *Client) SettingAuthentication(si *network.ServerIdentity, clientPriv kyber.Scalar, auth int) onet.ClientError {
	sig, err := schnorr.Sign(Suite, clientPriv, []byte{byte(auth)})
	if err != nil {
...
func (c *Client) DelFollow(si *network.ServerIdentity, clientPriv kyber.Scalar, sbid SkipBlockID) onet.ClientError {
	return nil
}
